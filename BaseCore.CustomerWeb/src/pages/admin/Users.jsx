    ...
    const openModal = (user = null) => {
        if (user) {
            ...
            setFormData({
                ...
                position: user.position || '', userType: user.userType || 0,
        ...
        try {
            if (editingUser) {
                const updateData = {
                    ...
                    position: formData.position, userType: parseInt(formData.userType),
                    isActive: formData.isActive,
                };
                if (formData.password) updateData.password = formData.password;
                await userApi.update(editingUser.id, updateData);
            } else {
                if (!formData.password) { setError('Password is required for new user'); return; }
                            ...
                            {loading ? (
                                ...
                                <>
                                            ...
                                            {users.length === 0 ? (
                                                ...
                                                users.map(user => (
                                                    <tr key={user.id}>
                                                        ...
                                                        <td>{user.phone}</td>
                                                        <td>
                                                            <span className={`badge ${user.userType === 1 ? 'badge-danger' : user.userType === 2 ? 'badge-warning' : 'badge-info'}`}>
                                                                {user.userType === 1 ? 'Quản trị' : user.userType === 2 ? 'Quản lý kho' : 'Người dùng'}
                                                            </span>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <button className="btn btn-sm btn-info mr-1" onClick={() => openModal(user)}>
                                    ...
                                    <div className="d-flex justify-content-between align-items-center">
                                        <span className="text-muted">Tổng: <strong>{totalCount}</strong> người dùng</span>
                                        <nav>
