...
export default function Cart() {
  ...
  // id sản phẩm đang chờ xác nhận xoá (null = không có)
  const [confirmRemoveId, setConfirmRemoveId] = useState(null)

  const handleDecrease = (item) => {
    if (item.quantity === 1) {
      // Thay vì xóa ngay, hiện xác nhận
            ...
            <tbody>
              {items.map((it) => (
                <>
                  ...
                  {/* Hàng xác nhận xoá — chỉ hiện khi đang hỏi sản phẩm này */}
                  {confirmRemoveId === it.id && (
                    <tr key={`confirm-${it.id}`} className="bg-red-50 border-t border-red-100">
                      <td colSpan={4} className="px-4 py-2.5">
                        <div className="flex items-center gap-3 text-sm">
                          <span className="text-red-600 font-medium">
                            Xoá &ldquo;{it.name}&rdquo; khỏi giỏ hàng?
                          </span>
                          <button
                            onClick={() => handleConfirmRemove(it.id)}
