...
const Products = () => {
    ...
    useEffect(() => {
        loadProducts();
    }, [page, keyword, categoryId, manufacturerId]);
    ...
    const loadProducts = async () => {
        ...
        try {
            ...
            if (!error.response) {
                setLoadError('Không thể kết nối API Gateway (port 5000). Hãy chắc chắn tất cả backend services đang chạy.');
            } else {
                setLoadError(`Lỗi tải sản phẩm (${error.response.status}): ${error.response.data?.message || 'Lỗi server nội bộ'}`);
            }
        } finally {
            setLoading(false);
        }
                                            ...
                                            {products.length === 0 ? (
                                                <tr>
                                                    <td colSpan={isAdmin() ? 7 : 6} className="text-center text-muted">
                                                        {loadError ? 'Lỗi tải dữ liệu' : 'Không tìm thấy sản phẩm'}
                                                    </td>
                                                </tr>
                                            ) : (
