...
export default function AdminProducts() {
  ...
  const [urlInputMode, setUrlInputMode] = useState(null) // productId
  ...
  const handleUrlSave = async (productId) => {
    const url = urlInputValue.trim()
    if (!url) {
      toast.error('Vui lòng nhập URL ảnh')
      return
    }
    setSavingUrl(productId)
    try {
      await productApi.update(productId, { imageUrl: url })
      setProducts((prev) =>
        prev.map((p) => (p.id === productId ? { ...p, imageUrl: url } : p))
      )
      toast.success('Cập nhật URL ảnh thành công!')
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 fade-in">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 text-sm text-blue-800">
        <b>Hướng dẫn:</b> Nhấn <b>📷 Chọn ảnh</b> để upload file, hoặc <b>🔗 Nhập URL</b> để dùng link ảnh trực tiếp. Nhấn <b>Sửa</b> ở cột giảm giá hoặc nhà sản xuất để cập nhật thông tin.
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-gray-600 w-52">Ảnh</th>
              </tr>
            </thead>
                      ...
                      {urlInputMode === p.id ? (
                        <div className="flex items-center gap-1.5">
                          <input
                            ...
                            onKeyDown={(e) => { if (e.key === 'Enter') handleUrlSave(p.id); if (e.key === 'Escape') { setUrlInputMode(null); setUrlInputValue('') } }}
                            autoFocus
                            className="w-40 border rounded px-2 py-1 text-xs outline-none focus:border-primary-500"
                          />
                          <button
                            onClick={() => handleUrlSave(p.id)}
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <button
                            onClick={() => { setUrlInputMode(p.id); setUrlInputValue(p.imageUrl?.startsWith('http') ? p.imageUrl : '') }}
                            className="bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs px-3 py-1.5 rounded font-medium border border-gray-300"
                          >
                            🔗 Nhập URL
                          </button>
                        </div>
