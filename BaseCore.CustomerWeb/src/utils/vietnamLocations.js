// Vietnam administrative locations after the 07/2025 two-level reform.
// Source snapshot: https://provinces.open-api.vn/api/v2/?depth=2
// Shape kept intentionally small for checkout: province/city -> ward/commune/special zone.
...
const _provinces = VIETNAM_LOCATIONS.map(({ code, name }) => ({ code, name }))
const _wardsByProvince = new Map(VIETNAM_LOCATIONS.map((p) => [p.code, p.wards.map((w) => w.name)]))
...
export async function fetchWards(provinceCode) {
  return _wardsByProvince.get(String(provinceCode)) || []
}

// Backward-compatible alias for old checkout code that used district wording.
export const fetchDistricts = fetchWards

export const VIETNAM_LOCATION_STATS = {
  provinceCount: 34,
  wardCount: 3321,
}
