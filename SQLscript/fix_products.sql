-- ============================================================
-- Fix Products: Khôi phục bảng Products và dữ liệu
...
-- Bước 1: Thêm cột DiscountPercent nếu chưa có
IF NOT EXISTS (
...
-- Bước 2: Thêm cột ApprovalNote vào Orders nếu chưa có (cho tính năng duyệt đơn)
IF NOT EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
...
-- Bước 3: Chèn dữ liệu Categories nếu bảng trống
...
-- Bước 4: Chèn dữ liệu Products nếu bảng trống
...
BEGIN
    ...
    PRINT '✓ Đã chèn 20 sản phẩm mẫu vào bảng Products';
END
ELSE
BEGIN
    PRINT '✓ Bảng Products đã có dữ liệu, bỏ qua bước chèn mới';
END
GO
